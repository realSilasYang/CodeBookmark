...
単一ファイルの設定、またはワークスペース全体の設定フォルダーをインポートできます。エクスポート形式は再インポート可能な JSON、階層テキスト、Markdown、HTML、CSV です。一括エクスポートは現在のフォルダーを再帰的に調べ、ブックマークがある各ソースファイルごとに個別の結果を作成します。

「ブックマーク設定ファイルを管理」では、スクリプト設定、現行レイアウト、旧順序記録、移行ログ、競合コピー、一時残留を確認して、安全な種類だけを削除できます。各情報欄はホバーで完全な値を表示します。中身がなく、Undo／Redo にも必要ないスコープフォルダーは自動的に削除されます。
...
Node.js 24 を使用します。`npm run verify` はコンパイル、ESLint、単体・契約・専門検証、`npm run test:integration` はローカル VS Code を再利用して 13 言語と英語フォールバック、`npm run check:release` はさらに依存監査と VSIX 内容を確認します。
