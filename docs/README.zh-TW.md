...
- 單一檔案模式可匯入一份 CodeBookmark 設定檔並綁定目前程式檔。
- 工作區模式可選擇整個書籤設定資料夾，匯入多個腳本與工作區版面配置。
- 匯出格式包括可再次匯入的 JSON、階層化純文字、Markdown、HTML 與 CSV。
- 批次匯出會遞迴掃描目前資料夾，只處理有書籤的程式檔，並為每個來源檔案建立獨立輸出。

「管理書籤設定檔」會列出腳本設定、工作區版面配置、舊版排序記錄、儲存遷移紀錄、衝突副本與暫存殘留。將游標停在腳本、工作區或記錄欄位即可查看完整資訊；清理功能只允許刪除白名單內的設定資料。作用域已沒有內容，且沒有任何可復原或重做操作時，空資料夾會自動刪除。
...
專案使用 Node.js 24。`npm run verify` 執行編譯、ESLint、單元測試、契約測試與所有專項驗證；`npm run test:integration` 重用本機 VS Code，在隔離環境驗證 13 種語言及英文回退；`npm run check:release` 再加入依賴稽核與 VSIX 清單檢查。
