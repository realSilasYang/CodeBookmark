...
When adding a release, copy the block below after the language links in `docs/CHANGELOG.en.md`. Keep only categories that contain actual changes. Each entry uses "**Feature name:** user-facing change and benefit" rather than commit messages, internal class names, or unrelated implementation details.
...
- Use `⚠️ Important Notes` only for upgrade actions, compatibility changes, or other information users must see in advance; when present, it must be the first section in the release.
