...
- 單檔模式可匯入一份 CodeBookmark 設定檔並綁定目前程式檔。
- 工作區模式可選擇完整書籤設定資料夾，匯入多個程式檔設定及工作區佈局。
- 匯出支援可再次匯入的 JSON、階層化文字、Markdown、HTML 及 CSV。
- 批次匯出只處理有書籤的程式檔，遞迴掃描目前資料夾，並為每個來源檔案輸出獨立結果。

「管理書籤設定檔」會列出程式檔設定、工作區佈局、舊排序記錄、遷移日誌、衝突副本及臨時殘留。腳本、工作區與記錄欄位可懸停查看完整內容；只有受控白名單內的設定資料可清理。作用域沒有內容且不再受撤銷／重做歷史保護時，空資料夾會自動刪除。
...
使用 Node.js 24。`npm run verify` 執行編譯、ESLint、單元測試、契約測試及全部專項驗證；`npm run test:integration` 重用本機 VS Code，在隔離使用者目錄測試 13 種語言及英語回退；`npm run check:release` 再加入依賴審計和 VSIX 清單檢查。

發佈只接受屬於 `main` 歷史的註解標籤。GitHub Actions 以 OIDC 短期憑證發佈 Marketplace，核對線上 VSIX 哈希，再建立包含 VSIX、CycloneDX SBOM 和 `SHA256SUMS` 的 GitHub Release。詳細流程見[發佈指南](https://github.com/realSilasYang/CodeBookmark/blob/main/docs/release/RELEASING.md)。
