...
단일 파일 설정이나 워크스페이스 설정 폴더 전체를 가져올 수 있습니다. 내보내기는 다시 가져올 수 있는 JSON, 계층형 텍스트, Markdown, HTML, CSV를 지원합니다. 일괄 내보내기는 현재 폴더를 재귀 탐색해 북마크가 있는 스크립트만 처리하고 원본 파일마다 별도 결과를 만듭니다.

“북마크 설정 파일 관리” 화면은 스크립트 설정, 현재 워크스페이스 배치, 이전 순서 기록, 저장소 이동 기록, 충돌 사본, 임시 잔여물을 보여 줍니다. 각 필드에 마우스를 올리면 전체 값을 볼 수 있으며 안전 목록에 포함된 설정 데이터만 삭제할 수 있습니다. 내용이 없고 실행 취소／다시 실행에도 필요하지 않은 scope 폴더는 자동으로 삭제됩니다.
...
Node.js 24를 사용합니다. `npm run verify`는 컴파일, ESLint, 단위·계약·전문 검증을 실행합니다. `npm run test:integration`은 설치된 VS Code를 격리 환경에서 재사용해 13개 언어와 영어 폴백을 검사합니다. `npm run check:release`는 의존성 감사와 VSIX 목록까지 확인합니다.
