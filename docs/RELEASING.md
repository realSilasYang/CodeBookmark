...
3. 为该身份创建 GitHub Environment 专用的联合凭据。Issuer、Subject 和 Audience 必须逐字一致；Environment 名称由工作流固定为 `marketplace-release`：
...
az identity federated-credential create \
  ...
  --subject repo:realSilasYang/CodeBookmark:environment:marketplace-release \
...
6. 在 GitHub Actions 中手动运行 `Resolve Marketplace Identity` 工作流。它只登录 Entra、调用 Azure DevOps Profile API，并把非机密的 Marketplace identity resource ID 写入运行摘要，不会构建或发布扩展。也可以从终端触发：

```bash
gh workflow run marketplace-identity.yml --repo realSilasYang/CodeBookmark
```
