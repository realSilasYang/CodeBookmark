...
- Every user-visible runtime string requires complete entries in all 13 registered catalogs. Keep the Simplified Chinese, Hong Kong Traditional Chinese, Taiwan Traditional Chinese, English, Japanese, Vietnamese, Korean, Spanish, French, Portuguese, Russian, German, and Italian wording native to each locale; unsupported non-Chinese locales fall back to English. Keep stable command IDs, protocol values, persistence fields, and decision logic independent of translated labels.
...
Each commit should express one complete intent and include proportionate tests. A pull request must explain the problem, behavior change, verification commands, and any effect on settings or persistence. User-visible changes update every affected README language and both changelog languages.
