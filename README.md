...
直接打开 [CodeBookmark 的 VS Code Marketplace 页面](https://marketplace.visualstudio.com/items?itemName=realSilasYang.codebookmark)，点击安装按钮并按提示交给 VS Code 完成安装。发布者应显示为 `realSilasYang`。扩展要求 VS Code `1.125.0` 或更高版本；版本过低时可前往 [VS Code 官方下载页](https://code.visualstudio.com/download)更新。

### 📦 从 VSIX 安装

打开 [CodeBookmark 最新版本发布页](https://github.com/realSilasYang/CodeBookmark/releases/latest)，在“Assets”中直接下载 `codebookmark-版本号.vsix`。然后在 VS Code 中执行“扩展：从 VSIX 安装…”，选择刚下载的文件；也可以在终端运行：

```bash
code --install-extension "D:\Downloads\codebookmark-版本号.vsix"
```

### 🧑‍💻 从源码运行

先安装 [Node.js 24](https://nodejs.org/en/download) 和 [VS Code](https://code.visualstudio.com/download)，再打开 [CodeBookmark 源码仓库](https://github.com/realSilasYang/CodeBookmark)获取源码。在项目目录执行 `npm ci` 与 `npm run compile`，然后按 `F5` 启动扩展开发宿主。完整的构建和测试命令见[开发者指南](#开发者指南)。
...
在面板中打开“更多 → 书签配置文件管理”，可以查看当前全局存储目录中的全部书签存储记录，而不受当前工作区或活动脚本限制。管理界面会明确区分三类内容：

- **脚本书签配置：** 显示脚本路径、绑定状态、配置类型、书签总数与各级数量、自动或失效书签数量、绑定信息更新时间、文件修改时间和大小，并区分正式配置、迁移备份、冲突副本、已取代文件及损坏文件。
- **工作区排序记录：** 显示工作区名称、路径哈希、参与排序的脚本路径及条目数量。这类记录只负责恢复工作区中的文件显示顺序，不保存书签内容。
- **存储迁移记录：** 显示迁移状态、原目录、目标目录、开始与完成时间，以及复制、合并和冲突文件数量。这类记录只用于说明最近一次存储目录迁移结果。

界面支持统一搜索、筛选、排序和多选；脚本配置可以打开仍存在的脚本，所有记录都可以在文件资源管理器中定位。删除或清理前，扩展会按所选类型说明影响并再次确认：删除脚本配置会直接删除其中的书签，不能通过书签撤销功能恢复；清理工作区排序记录不会删除书签，但对应工作区的自定义文件顺序会被重置；清理存储迁移记录不会影响当前书签或存储目录。确认后会先完成等待中的保存任务，再核对文件是否仍与界面读取时一致；已被其他程序修改的文件会跳过，防止误删新内容。操作完成后，管理界面和当前书签树都会从磁盘重新载入。
...
修改 `globalStoragePath` 后，扩展会先把旧目录中等待保存的数据全部写完，再将 `scripts`、`scopes` 和恢复日志合并到新目录。目标目录中不同的数据会保留为备份或冲突副本；只有新目录全部写入成功后，扩展才会删除原目录中的书签配置。转移或清理失败时，扩展会继续使用原目录，不会提前删除来源数据。
...
├─ tests/integration/               VS Code Extension Host 集成测试
...
存储根切换由 `StorageRootTransfer` 串行执行：先刷新来源保存队列，再逐文件复制或合并。相同脚本身份按 `lastSeenAt` 选择主数据并保留无重复书签；同身份冲突会重写书签身份；无法语义合并的文件保存为带内容哈希的冲突副本。目标数据全部落盘后，来源根中的 `scripts`、`scopes`、`.script-relocations` 和迁移日志会被删除；根目录内不属于 CodeBookmark 的文件保持不变。真实路径检查阻止来源和目标通过符号链接或目录联接互相包含。
...
- `npm run test:integration`：编译后自动查找并复用本机已安装的 VS Code，以隔离的临时用户目录启动 Extension Host，验证中英文环境下的真实激活、命令注册、配置和文件打开；找不到本机程序时明确失败，不会下载额外测试运行时。
...
当前专项验证覆盖激活时序、AI 地址归一化、五类 AI 协议、同源路由回退、密钥、大小与取消，以及自动标记、导入导出、命令清单、存储根转移、移动重连、外部配置、保存队列、作用域、撤销、视图切换和图标资源。新增行为应优先补到对应 `verify-*.js`；涉及 VS Code API 生命周期的行为再补集成测试。
...
发布前应通过 `npm run check:release`。扩展包只包含 `out`、`resources`、`package.nls*.json`、根目录中文 `README` 与 `CHANGELOG`、`docs` 中的英文版本、`LICENSE` 和 `docs/legal` 下的第三方声明与许可证。推送版本标签后，Release 工作流会通过 GitHub OIDC 与 Microsoft Entra ID 的短期令牌，用同一个 VSIX 依次发布 Marketplace 和 GitHub Release，仓库不保存长期发布凭据；身份配置、失败重跑与完整发布步骤见 [发布指南](https://github.com/realSilasYang/CodeBookmark/blob/main/docs/release/RELEASING.md)。Marketplace Publisher ID 已固定为 `realSilasYang`，并由发布工作流核对，后续版本不应修改。项目源码使用 MIT 许可证，第三方图标与 Fuse.js 遵循各自许可证。
