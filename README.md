...
├─ docs/
│  ├─ images/                       README 界面截图
│  ├─ CHANGELOG_TEMPLATE*.md        中英文结构化更新日志模板
│  └─ RELEASING*.md                 中英文维护者发布指南
├─ src/
...
├─ scripts/
│  ├─ build-release-notes.js        从 CHANGELOG 生成中文 Release 正文
│  ├─ manifest-localizations.js     生成清单所需的英文译文
...
├─ package.nls*.json                生成的默认英文和中文清单文本
├─ CHANGELOG.md / CHANGELOG.en.md   中英文版本变化记录
└─ THIRD_PARTY_NOTICES.md           内置图标、Fuse.js 的来源与署名
```

`out/`、`.vscode-test/` 和 `node_modules/` 都是生成或依赖目录，不应手工修改。扩展元数据和 npm 脚本定义在 `src/util/constants/BasePackage.ts`；命令、菜单、快捷键、设置和子菜单定义在 `src/util/constants/Commands.ts`；颜色定义在 `Colors.ts`。`npm run compile` 会先清理 `out/`、编译 TypeScript，再重新生成 `package.json` 和三份 `package.nls*.json`。
...
- 如需指定其他 VS Code，可运行 `node scripts/run-integration-tests.js "--vscode-executable=<Code.exe 路径>"`，或设置 `CODEBOOKMARK_VSCODE_EXECUTABLE_PATH`；显式路径优先于自动发现。
...
发布前应通过 `npm run check:release`。扩展包只包含 `out`、`resources`、`package.nls*.json`、中英文 `README` 与 `CHANGELOG`、`LICENSE`、`THIRD_PARTY_NOTICES.md` 和 `THIRD_PARTY_LICENSES`。推送版本标签后，Release 工作流会通过 GitHub OIDC 与 Microsoft Entra ID 的短期令牌，用同一个 VSIX 依次发布 Marketplace 和 GitHub Release，仓库不保存长期发布凭据；身份配置、失败重跑与完整发布步骤见 [发布指南](https://github.com/realSilasYang/CodeBookmark/blob/main/docs/RELEASING.md)。Marketplace Publisher ID 已固定为 `realSilasYang`，并由发布工作流核对，后续版本不应修改。项目源码使用 MIT 许可证，第三方图标与 Fuse.js 遵循各自许可证。
