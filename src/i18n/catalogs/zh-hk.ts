/**
 * 香港繁體中文目錄按本地常用的「檔案、資料夾、設定、圖示」等用語重新編寫。
 * 協定欄位、命名佔位符和技術識別碼須保持原樣，以免本地化改變功能行為。
 */
import { messages as defaultMessages } from './zh-cn'

const iconSemanticCatalog = `- entry：程式進入點、啟動、初始化
- algorithm：具名演算法、編解碼、雜湊、排序或壓縮
- flow：工作流程、生命週期、處理管線、狀態機
- branch：條件分支、路由分派、策略選擇
- architecture：軟件架構、框架、核心引擎
...
- authentication：身分驗證、鑑權、登入、密鑰、權杖、憑證
- encryption：加密、解密、密碼學、密文
- privacy：私隱保障、資料遮蔽、PII、GDPR
- locking：互斥鎖、讀寫鎖、信號量、臨界區、死鎖
- unlocking：解鎖、釋放鎖定、解除凍結
- ai：人工智能、大型語言模型、模型推理、提示詞、代理
- calculation：數學計算、公式、算術、計費
- policy：政策管治、合規、政策、稽核規則
- documentation：文件、README、手冊、使用指南
- image：圖像、圖片、畫布、點陣圖、縮圖
- audio：音訊、聲音、語音、錄音
- video：影片、錄像、媒體串流、視訊編解碼
- user：使用者、帳戶、個人資料、租戶
...
export const messages = {
	...
	'undoAction.reorderFiles': '調整檔案次序',
...
${iconSemanticCatalog}`,
	...
	'providers.BookmarkConfigurationManagementController.theCorrespondingScriptDoesNotExistAndCannotBe': '相應指令碼不存在，無法開啟。',
	...
	'providers.BookmarkConfigurationManagerWebview.restoresScriptDisplayOrderForThisWorkspace': '用來復原此工作區的指令碼顯示次序',
	...
	'providers.BookmarkConfigurationManagerWebview.unableToIdentifyTheCorrespondingScript': '無法識別相應指令碼',
	...
	'providers.BookmarkConfigurationManagerWebview.workspaceOrderRecordsAffectsFileOrderOnlyBookmarksAre': '工作區排序記錄：{count} 筆（只影響檔案次序，不會刪除書籤）',
	...
	'providers.BookmarkTreeInteractionRunner.chooseTheViewOrderDoesNotChangeTheUnderlying': '選擇檢視排序方式（不影響拖放所用的原始次序）',
	...
	'providers.CodeBookmarkViewProvider.bookmarkConfigurationWatcherFailed': '書籤設定監察器失敗（{directory}）: {errorMessage}',
	...
	'providers.CodeBookmarkViewProvider.failedToReadTheWorkspaceBookmarkOrder': '讀取工作區書籤次序失敗: {errorMessage}',
	...
	'providers.CodeBookmarkViewProvider.failedToRestoreTheBookmarkConfigurationWatcher': '復原書籤設定監察器失敗: {errorMessage}',
	...
	'providers.CodeBookmarkViewProvider.failedToSetUpTheBookmarkConfigurationWatcher': '設定書籤設定監察器失敗: ',
	'providers.CodeBookmarkViewProvider.failedToSetUpTheBookmarkConfigurationWatcher2': '設定書籤設定監察器失敗: {errorMessage}',
	...
	'providers.CodeBookmarkViewProvider.unableToSaveTheRestoredWorkspaceFileOrderCheck': '無法儲存復原後的工作區檔案次序，請檢查書籤儲存路徑權限。',
	...
	'providers.UndoManager.undoWorkspaceOrderIsInvalid': '撤銷工作區次序無效',
	...
	'repository.BookmarkRepository.failedToCleanTheWorkspaceOrderAfterDeletingA': '刪除書籤設定後，清理工作區次序失敗（{scriptPath}）: {error}',
	...
	'repository.BookmarkRepository.unableToUpdateTheWorkspaceBookmarkOrder': '無法更新工作區書籤次序',
	...
	'util.AIService.preparingTheAiNetworkRequest': '正在準備傳送至模型的網絡請求…',
	...
	'util.LanguageCommentProfiles.languageCommentConfigurationsCouldNotBeReadTheAffected': '有 {failedConfigurations} 個語言註解設定無法讀取；相應語言不會參與自動標記識別。',
