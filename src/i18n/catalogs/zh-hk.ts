...
${iconSemanticCatalog}`,
	...
	'util.PerformanceMonitor.bookmarks': '書籤數目',
	...
	'util.PerformanceMonitor.workspaceCodeMarkerScanDetails': '[效能] {name}\n  實際耗時：總計 {totalMs} 毫秒；搜尋檔案 {discoveryMs} 毫秒；處理檔案 {processingMs} 毫秒\n  檔案統計：候選 {files}；已找到 {discoveredFiles}；已開啟文件 {openedDocuments}；讀取資料 {readMiB} MiB\n  並行累計耗時（不可與實際耗時相加）：搜尋查詢 {discoveryQueryMs} 毫秒（{discoveryQueries} 次）；開啟檔案 {openMs} 毫秒；讀取檔案 {readMs} 毫秒；精確掃描 {exactScanMs} 毫秒\n  掃描結果：預先篩除 {prefilteredFiles}（{prefilterRate}%）；精確掃描 {exactScans}（{exactRate}%）；發生變更 {changedFiles}',
	...
	"commands.exportCommand.chooseCurrentFolderRootDescription": "目前沒有使用中的指令碼，請選擇要匯出的根目錄",
	...
	"providers.PortableImportWorkflowRunner.noUniqueTargets": "找不到可唯一繫結的本機指令碼；{count} 個指令碼出現配對衝突。請開啟對應資料夾或指令碼後再試。",
	...
	"providers.BookmarkConfigurationManagementController.portableExchangeRecords": "{deletedPortableExchanges} 項可攜式設定交換記錄",
	...
	"providers.BookmarkConfigurationManagerWebview.portableExchangeRecordsRemovalEffect": "可攜式設定交換記錄：{count} 項（清理後不會刪除目前書籤，但會失去相關交換系列的身分對應及三方合併基準）",
	"providers.BookmarkConfigurationManagerWebview.exchangeIdentity": "交換系列：{value}",
	"providers.BookmarkConfigurationManagerWebview.exchangeScope": "綁定範圍：{value}",
	...
	"providers.BookmarkConfigurationManagerWebview.exchangePurpose": "用於跨裝置來回分享時延續身分、冪等匯入及三方合併",
	"providers.BookmarkConfigurationManagerWebview.exchangeUpdated": "交換記錄更新：{date}",
	"providers.PortableImportWorkflowRunner.invalidPackage": "所選檔案不是受支援的可攜式書籤設定；檔案可能已損壞、格式錯誤，或由較新版本的代碼書籤建立。",
