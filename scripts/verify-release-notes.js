...
try {
  ...
  for (const heading of ['## 🎉 版本 X.Y.Z - YYYY-MM-DD', '### ⚠ 重要说明', '### ✨ 新增', '### 🚀 优化', '### 🐛 修复']) {
    assert.ok(template.includes(heading), `更新日志模板缺少结构：${heading}`)
  }
  assert.ok(
    template.indexOf('### ⚠ 重要说明') < template.indexOf('### ✨ 新增'),
    '重要说明必须是模板中的第一个版本分类',
  )
  ...
  for (const version of versions) {
    ...
    assert.match(releaseNotes, /^## (?:⚠ 重要说明|✨ 新增|🚀 优化|🐛 修复)$/m)
    const importantNotesIndex = releaseNotes.indexOf('## ⚠ 重要说明')
