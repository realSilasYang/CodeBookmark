/**
 * 锁定真实 Extension Host 日志的已知外部诊断分类，项目错误不能被宽泛过滤。
 * 脚本围绕“锁定真实 Extension Host 日志的已知外部诊断分类”构造最小输入，同时覆盖正常路径和明确拒绝的边界。
 */
...
assert.throws(
  () => assertNoUnexpectedExtensionHostDiagnostics(
    'Failed to create default profile extensions manifest in extensions installation folder. CodeBookmark activation failed',
    '',
  ),
  /Unexpected Extension Host diagnostics/,
)
