/**
 * 用多种注释语法和反例锁定 TODO/FIXME/BUG 显式指令识别，普通文字与 SVG 元数据不得命中。
 * 脚本直接调用编译后的 `Bookmark`、`BookmarkSet`、`CodeMarkerScanner`，只在 VS Code 或文件系统边界使用最小替身。
 */
...
assert.match(workflowControllerSource, /await scanWorkspaceCodeMarkers\(scope, generation, CODE_MARKER_SCAN_CONCURRENCY/)
...
assert.match(workspaceScanSource, /prefilteredEmpty && !knownMarkerFile/)
assert.match(workspaceScanSource, /port\.measure\(startedAt, \{/)
...
assert.match(providerSource, /providers\.CodeBookmarkViewProvider\.todoFixmeBugBookmarksAreManagedAutomaticallyFromSource/)
assert.match(deletionRunnerSource, /targets = targets\.filter\(target => !target\.isCodeMarker\)/)
assert.match(deletionRunnerSource, /preserveProtectedMarkers\(targets, port\)/)
