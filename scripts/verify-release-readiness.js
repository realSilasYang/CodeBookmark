...
assert.equal('sbom' in manifest.scripts, false)
assert.equal('release:build' in manifest.scripts, false)
...
assert.doesNotMatch(manifest.scripts['check:release'], /package:vsix/)
...
const packageVsixScript = read(path.join('scripts', 'release', 'package-vsix.js'))
...
assert.match(packageVsixScript, /runner temp directory/)
...
assert.doesNotMatch(releaseWorkflow, /attestations: write/)
...
assert.doesNotMatch(releaseWorkflow, /CycloneDX|SHA256SUMS|SBOM|npm run sbom|write-sha256sums|attest-build-provenance|attest-sbom/)
assert.doesNotMatch(releaseWorkflow, /SBOM_PATH|SUMS_PATH|\$sbom|\$sums/)
...
assert.match(releaseWorkflow, /\$vsix --clobber/)
assert.match(releaseWorkflow, /gh release create \$env:GITHUB_REF_NAME \$vsix `/)
