...
assert.equal(commandsById.get('codebookmark.ai.generateOverwriteFolder').title, menuTitle('↻', '为有书签的脚本重新生成并替换'))
assert.equal(commandsById.get('codebookmark.ai.generateAppendFolderDirect').title, menuTitle('⊕', '为当前文件夹内有书签的脚本追加'))
assert.equal(commandsById.get('codebookmark.ai.generateOverwriteFolderDirect').title, menuTitle('↻', '为当前文件夹内有书签的脚本重新生成并替换'))
assert.equal(commandsById.get('codebookmark.ai.generateSkipFolder').title, menuTitle('✦', '为所有无书签脚本生成'))
assert.equal(commandsById.get('codebookmark.ai.generateSkipFolderDirect').title, menuTitle('✦', '为当前文件夹内无书签脚本生成'))
assert.equal(commandsById.get('codebookmark.ai.optimize').title, menuTitle('✎', '当前脚本'))
assert.equal(commandsById.get('codebookmark.ai.optimizeDirect').title, menuTitle('✎', '优化当前脚本的书签标签'))
assert.equal(commandsById.get('codebookmark.ai.optimizeFolderDirect').title, menuTitle('✎', '优化当前文件夹内有书签的脚本中的书签标签'))
...
assert.equal(submenusById.get('codebookmark.aiGenerateFileSubmenu').label, menuTitle('▧', '当前脚本'))
assert.equal(submenusById.get('codebookmark.aiGenerateFolderSubmenu').label, menuTitle('▣', '当前文件夹'))
assert.equal(submenusById.get('codebookmark.aiGenerateWorkspaceSubmenu').label, menuTitle('✦', '生成书签'))
assert.equal(submenusById.get('codebookmark.aiOptimizeSubmenu').label, menuTitle('✎', '优化书签标签'))
...
const vscodeDocumentSource = fs.readFileSync('src/util/VscodeDocument.ts', 'utf8')
...
assert.match(selectedRunnerSource, /readAISourceSnapshot\(filePath, findOpenFileDocument\)/)
assert.ok((folderRunnerSource.match(/readAISourceSnapshot\(filePath, findOpenFileDocument\)/g) || []).length >= 2)
assert.match(vscodeDocumentSource, /document\.uri\.scheme === 'file'/)
assert.match(vscodeDocumentSource, /normalizedAbsolutePath\(document\.uri\.fsPath\) === normalizedPath/)
