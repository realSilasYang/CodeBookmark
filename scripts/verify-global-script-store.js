/**
 * 核对全局单文件索引的当前格式写入、旧格式迁移和无版本字段清理。
 * 为核对全局单文件索引的当前格式写入、旧格式迁移和无版本字段清理，脚本在临时目录中调用编译后的 `Bookmark`、`BookmarkSet`、`ContextValue` 完成真实操作，检查落盘结果而不是内存假象。
 */
...
async function main() {
  const workspaceScopeFolder = fileUtils.getWorkspaceBookmarkFolder(workspaceRoot, storageRoot)
  assert.equal(fs.existsSync(workspaceScopeFolder), false, 'resolving a scope path must not create an empty folder')
  ...
  assert.equal(fs.existsSync(workspaceScopeFolder), false, 'script-only persistence does not need scope metadata')
